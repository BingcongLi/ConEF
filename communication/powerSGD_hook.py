...
from compressors.CountSketch import DenseCS
        ...
        if compress_test[0]:
            ...
            if state.use_error_feedback and state.use_error_compression:
                ...
                if cnt not in state.error_dict[bucket_index]:
                    state.error_dict[bucket_index][cnt] = DenseCS(n, m, state.sketch_size)
        ...
        if state.use_error_feedback:
            ...
            if not state.use_error_compression:
                ...
                for tensor, cp_tensor in zip(tensors_to_compress, copy_list):
                    cnt += 1
                    cp_tensor.add_(-1, tensor)
                    state.error_dict[bucket_index][cnt].update(cp_tensor, state.beta)
    ...
    for tensor in tensors:
        ...
        if compress_test[0]:
            ...
            if state.use_error_feedback and state.use_error_compression:
                # initialize count sketch
                if cnt not in state.error_dict[bucket_index]:
                    state.error_dict[bucket_index][cnt] = DenseCS(m, n, state.sketch_size)
                # add compressed error back to matrix
                state.error_dict[bucket_index][cnt].query(matrix.view(m, n), 1 - state.beta)
                copy_list.append(torch.clone(matrix).detach())
        ...
        if state.use_error_feedback:
            ...
            if not state.use_error_compression:
                ...
                for tensor, cp_tensor in zip(tensors_to_compress, copy_list):
                    ...
                    n, m = tensor.shape
                    cp_tensor.add_(-1, tensor)
                    state.error_dict[bucket_index][cnt].update(cp_tensor.view(m, n), state.beta)
