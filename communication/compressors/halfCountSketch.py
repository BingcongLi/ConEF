...
{   
	...
	for(int index = threadIdx.x; index < D; index += blockDim.x)
	{
		...
		// int hash_idx = hash(offset + index, W, a, b);
...
class DenseCS:
    def __init__(self, N, D, sketch_size):
        ...
        self.cs = torch.HalfTensor(self.N, self.range).fill_(0).to(device)
        self.kernel = None
        self.query_kernel = None
        self.a = torch.cuda.IntTensor(1).fill_(994443)
        self.b = torch.cuda.IntTensor(1).fill_(609478)
    ...
    def update(self, vec, beta):
        ...
        self.cs = self.cs.to(torch.float32)
        self.initialize()
        ...
        self.cs = self.cs.to(torch.float16)
