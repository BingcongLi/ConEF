...
from optimizers.funsed_lans import FusedLANS
...
def parse_arguments():
    ...
    parser.add_argument('--optimizer', type=str, default='LAMB',
                        help='Using LAMB as optimizer')


    ## gradient compression
    parser.add_argument('--grad_reducer',
                        default='allreduce',
                        type=str,
                        help='gradient compressor, valid choices:'
                             'allreduce'
                             'conef_randomblock'
                             'ef_randomblock'
                             'unbiased_randomblock')

    parser.add_argument('--start_compression_iter',
                        default=10,
                        type=int,
                        help='iteration to start gradient compression')

    parser.add_argument('--start_compression_percentage',
                        default=0.01,
                        type=float,
                        help='iteration to start gradient compression')

    # (unbiased) random block k setups
    parser.add_argument('--compression_ratio',
                        default=0.1,
                        type=float,
                        help='compression ratio')

    # ConEF setups
    parser.add_argument('--sketch_size',
                        default=0.2,
                        type=float,
                        help='sketch_size for ConEF')
    ...
    args.start_compression_iter = int(args.max_steps * args.start_compression_percentage)
...
def prepare_model_and_optimizer(args, device):
    ...
    # put the files in optimizers in to apex dependencies!
    if args.optimizer.lower() == 'lamb':
        optimizer = FusedLAMB(optimizer_grouped_parameters, lr=args.learning_rate)
    elif args.optimizer.lower() == 'lans':
        optimizer = FusedLANS(optimizer_grouped_parameters, lr=args.learning_rate, normalize_grad=False)
    else:
        raise ValueError('Unsupported optimizer.')

    # lr_scheduler = PolyWarmUpScheduler(optimizer, warmup=args.warmup_proportion, total_steps=args.max_steps)
    ...
    if args.local_rank != -1:
        if not args.allreduce_post_accumulation:
            ...
            comm_state = get_comm_hooks(args, model)
        else:
...
def take_optimizer_step(args, optimizer, model, overflow_buf, global_step):
    ...
    if args.allreduce_post_accumulation:
        # we will not use all_reduce_post_accumulation
